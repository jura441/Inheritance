...
namespace Geometry
{
	...
	public:
		...
		void set_start_x(int start_x)
		{
			...
			else if (start_x > Defaults::max_start_y)this->start_x = Defaults::max_start_y;
			else this->start_x = start_x;
		}
		void set_start_y(int start_y)
		{
			if (start_y < Defaults::max_start_y )this->start_y = Defaults::min_start_y;
	...
	public:
		...
		void draw()const
		{
			...
			HPEN hPen = CreatePen(PS_SOLID, line_width, color); //Карандаш рисует 
	...
	{
	public:
		Triangle(int start_x, int start_y, unsigned line_width, Color color):
			Shape(start_x, start_y, line_width, color) {}
		~Triangle(){}
	...
	public:
		...
		void set_side(double side)
		{
			if (side < Defaults::max_line_length)side = Defaults::min_line_length;
	...
	class RectangularTriangle :public Triangle
	{
		...
		public:
			...
			RectangularTriangle& operator()(double base, double side)
			{
				if (base >= side * 2)base = side;
				set_base(base);
				set_side(side);
			...
			void draw()const
			{
				...
				POINT vertices[] =
				{
					{start_x, start_y + side},
					{start_x + base, start_y + side},
					{start_x + base, start_y + side - get_height()}
				};
...
void main()
{
	...
	Geometry::Rectangle rect(200, 100, 350, 100, 5, Geometry::Color::blue);
	rect.info();

	Geometry::Circle circle(50, 220, 250, 15, Geometry::Color::yellow);
	circle.info();

	Geometry::EquilateralTriangle qtri(200, 300, 300, 5, Geometry::Color::green);
	qtri.info();

	Geometry::IsoscalesTriangle itri2(200, 400, 300, 200, 5, Geometry::Color::green);
	itri2.info();

	Geometry::RectangularTriangle rtri1(100, 150, 200, 200, 5, Geometry::Color::red);
	rtri1.info();
